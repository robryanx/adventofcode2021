...
func main() {
	...
	board_winning_order := make([]int, len(boards))

	for i := 0; i < len(draw_order); i++ {
		for j := 0; j < len(boards); j++ {
			if board_winning_order[j] == 0 {
				board, is_winner := check_board(boards[j], draw_order[0:i])

				if is_winner {
					board_winning_order[j] = board_winning_count
					if board_winning_count == len(boards) {
						fmt.Println(board_winning_count)
